package me.zeanzai.sharding.sql.controller;

import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import lombok.extern.slf4j.Slf4j;
import me.zeanzai.sharding.sql.bean.request.order.UserOrderInfoRequestDTO;
import me.zeanzai.sharding.sql.bean.web.DataResponse;
import me.zeanzai.sharding.sql.bean.web.OperationResponse;
import me.zeanzai.sharding.sql.bean.web.PageResponse;
import me.zeanzai.sharding.sql.common.enums.OrderCode;
...
public class UserOrderController {
    ...
    @GetMapping("/getOrderItemDetailToMakeOOM")
    public DataResponse getOrderItemDetailToMakeOOM(){
        try{
            //开始计时
            long bTime = System.currentTimeMillis();
            List<OrderInfo> all = userOrderInfoService.getAll();
            //关闭分段计时
            long eTime = System.currentTimeMillis();
            //输出
            log.info("查询用户订单明细耗时："+(eTime-bTime) + "ms");
            return DataResponse.success(all);
        }catch (Exception e){
            return DataResponse.error(OrderCode.QUERY_ORDER_ERROR.getDesc());
        }
